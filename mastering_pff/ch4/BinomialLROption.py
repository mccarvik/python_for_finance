...
class BinomialLROption(BinomialTreeOption):
    ...
    def _setup_parameters_(self):
        ...
        # d1 and d2 same as black scholes
        ...
        d2 = (math.log(self.S0/self.K) +
              ((self.r-self.div) -
               (self.sigma**2)/2.) *
              self.T) / (self.sigma * math.sqrt(self.T))
        # discrete approximation to the cummulative distribution function for a normal distribution
        # Leisen and Reimer developed a model with the purpose of improving 
        # the rate of converegence of their binomial tree
        # ie: dont need to lower the time steps as much
        pp_2_inversion = \
            lambda z, n: \
            .5 + math.copysign(1, z) * \
