...
class BlackScholes:
    ...
    def d1(self, S0, K, r, T, sigma, div):
        return (np.log(S0/K) + ((r-div) + sigma**2 / 2) * T)/ (sigma * np.sqrt(T))
    ...
    def call_pricer(self, S0, K, r, T, sigma, div):
        ...
        return S0 * np.exp(-div * T) * stats.norm.cdf(d1) - K * np.exp(-r * T) * stats.norm.cdf(d2)

    def put_pricer(self, S0, K, r, T, sigma, div):
        d1 = self.d1(S0, K, r, T, sigma, div)
        d2 = self.d2(S0, K, r, T, sigma, div)
        return K * np.exp(-r * T) * stats.norm.cdf(-d2) - S0 * np.exp(-div * T) *stats.norm.cdf(-d1)
