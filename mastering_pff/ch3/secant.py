...
def secant(f, a, b, tol=0.001, maxiter=100):
    ...
    n = 1
    while n <= maxiter:
        # move down the function f from a start point drawing secants getting closer to the root
        # each root of the secant, sets the point on f for the next secant line
        c = b - f(b)*((b-a)/(f(b)-f(a)))
        if abs(c-b) < tol:
            return c, n
