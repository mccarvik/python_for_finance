...
class MeanRevertingStrategy(Strategy):
    ...
    def event_tick(self, market_data):
        ...
        # see if z score is higher than threshold indicating an order
...
class Backtester:
    def __init__(self, symbol, start_date, end_date, data_source="yahoo"):
    ...
    def print_position_status(self, symbol, prices):
        if symbol in self.positions:
            ...
            print(self.get_trade_date(), "Net:", position.net, "Value:", position.position_value, "UPnL:", position.unrealized_pnl, "RPnL:", position.realized_pnl)
...
if __name__ == "__main__":
    ...
    plt.savefig(IMG_PATH + 'realized.png', dpi=300)
    plt.close()

    backtester.upnl.plot()
    plt.savefig(IMG_PATH + 'unrealized.png', dpi=300)
    plt.close()