...
from dawp.ch10.european_mcs import SRD_generate_paths
...
for alpha in it.product(py_list, x_disc_list, m_list, paths_list, control_variate, anti_paths, moment_matching):
    print('\n\n', alpha, '\n')
    ...
    for run in range(runs):  # simulation runs
        for panel in range(4):  # panels
            if verbose:
                print("\nResults for Panel %d\n" % (panel + 1))
                print(tmpl_1 % ('T', 'K', 'V0', 'V0_LSM', 'V0_CV', 'P0', 'P0_MCS', 'err', 'rerr', 'acc1', 'acc2'))
