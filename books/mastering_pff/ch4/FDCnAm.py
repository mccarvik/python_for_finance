...
class FDCnAm(FDCnEu):
    ...
    def __init__(self, S0, K, r, T, sigma, Smax, M, N, omega, tol, is_call=True):
        ...
        # over-relaxation parameter --> high omega = faster convergence, 
        # but also higher possibilities of algorithm not converging and vice versa
        self.omega = omega
    ...
    def _traverse_grid_(self):
        ...
        # Guass Seidel Model --> NO IDEA WHATS GOIN ON HERE
        # Somehow uses the boundary conditions with the early exercise of the American option
        for j in reversed(range(self.N)):
            ...
            while self.tol < error:
                new_values[0] = max(self.payoffs[0], old_values[0] + self.omega/(1-self.beta[1]) *
                        (rhs[0] -(1-self.beta[1])*old_values[0] + (self.gamma[1]*old_values[1])))

                for k in range(self.M-2)[1:]:
                    new_values[k] = \
                        max(self.payoffs[k], old_values[k] + self.omega/(1-self.beta[k+1]) *
                            (rhs[k] + self.alpha[k+1]*new_values[k-1] - (1-self.beta[k+1]) * 
                            old_values[k] + self.gamma[k+1]*old_values[k+1]))

                new_values[-1] = \
                    max(self.payoffs[-1], old_values[-1] + self.omega/(1-self.beta[-2]) *
