...
#
def SRD_generate_paths(x0, kappa, theta, sigma, T, M, I, rand, row, cho_matrix):
...
#
def B96_generate_paths(S0, r, v, lamb, mu, delta, rand, row1, row2, cho_matrix, T, M, I, moment_matching):
...
def plot_rate_paths(r):
    ...
    plt.savefig(PNG_PATH + 'rate_path.png', dpi=300)
    plt.close()
...
def plot_volatility_paths(v):
    ...
    plt.savefig(PNG_PATH + 'vol_path.png', dpi=300)
    plt.close()
...
def plot_index_paths(S):
    ...
    plt.savefig(PNG_PATH + 'index_path.png', dpi=300)
    plt.close()
...
def plot_index_histogram(S):
    ...
    plt.savefig(PNG_PATH + 'index_hist.png', dpi=300)
    plt.close()
