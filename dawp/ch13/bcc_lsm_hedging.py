...
from dawp.ch12.simulation import *
from dawp.ch13.bsm_lsm_hedging import plot_hedge_path
...
def BCC97_hedge_run(p):
    ...
    print("---------------------------------------")
    ds = dis * S0
    V_1, S, r, v, ex, rg, h, dt = BCC97_lsm_put_value(S0 + (2 - a) * ds, K, T, M, I)
    ...
    print(82 * "-")
    print("step|" + 7 * " %9s|" % ('S_t', 'Port', 'Put','Diff', 'Stock', 'Bond', 'Cost'))
...
if __name__ == '__main__':
    results = BCC97_hedge_run(1)
    plot_hedge_path(results[0], results[1], results[2], results[3], results[4], 'bcc_')
