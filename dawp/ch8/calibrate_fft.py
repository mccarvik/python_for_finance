...
from valuation_fft import M76_value_call_FFT
...
def M76_error_function_FFT(p0):
    ...
    for row, option in options.iterrows():
        ...
        model_value = M76_value_call_FFT(S0, option['Strike'], T, r, sigma, lamb, mu, delta)
        # standard error
...
def generate_plot(opt, options):
    ...
    for i, mat in enumerate(mats):
        options[options['Maturity'] == mat][['Call', 'Model']].plot(style=['b-', 'ro'], title='%s' % str(mat)[:10], grid=True)
        plt.ylabel('option value')
        plt.savefig(PNG_PATH + 'M76_calibration_3_%s.png' % i, dpi=300)
        plt.close
