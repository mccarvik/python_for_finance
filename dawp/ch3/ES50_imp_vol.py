...
# Option Data
#
data = pd.HDFStore('../data/es50_option_data.h5', 'r')['data']
...
def calculate_imp_vols(data):
    ...
    pdb.set_trace()
    data['Imp_Vol'] = 0.0
    data['Date'] = data['Date'].apply(lambda x: datetime.datetime.fromtimestamp(x / 1e9))
    data['Maturity'] = data['Maturity'].apply(lambda x: datetime.datetime.fromtimestamp(x / 1e9))
...
def plot_imp_vols(data):
    ...
    plt.savefig(PNG_PATH + "plot_imp_vols", dpi=300)
    plt.close()
