...
plt.savefig(PNG_PATH + "2d_output_imp_vol", dpi=300)
plt.close()
...
fig = figure()
pltt = p3.Axes3D(fig)
pltt.plot_wireframe(k, t, imv)
pltt.set_xlabel('K')
pltt.set_ylabel('T')
pltt.set_zlabel('Implied Volatility')
plt.savefig(PNG_PATH + "3d_output_imp_vol", dpi=300)
plt.close()


c = call_option(S0, K[j], dt.datetime(2014,1,1), dt.datetime(2014,7,1), r[0], 0.2)
# available functions
print(bsm_call_value(S0, K[j], T[0], r[0], 0.2))
print(bsm_vega(S0, K[j], T[0], r[0], 0.2))
print(bsm_call_imp_vol(S0, K[j], T[0], r[0], C1[j], 0.2))

# internal call functions
print(c.value())
print(c.vega())
print(c.imp_vol(C1[j]))